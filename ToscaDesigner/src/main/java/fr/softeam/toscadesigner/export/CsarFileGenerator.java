...
public class CsarFileGenerator extends AbstractToscaFileGenerator {
	...
	private void addDefinitions(ZipOutputStream zos, MObject toscaModel) throws IOException {
		...
		for (MObject element : modelPackage.getCompositionChildren()) {
			...
			// If it's a node type, or a topology template, render the .tosca file
			if (new NodeTypeChecker().isTypeOf(element) || new TopologyTemplateChecker().isTypeOf(element)) {
			...
			// If not, check if it's an embedded ToscaModel, TServiceTemplate or a
			// ToscaGroup. If so, do a
			// recursive call to render its contents.
			if (new ToscaModelChecker().isTypeOf(element) || new TServiceTemplateChecker().isTypeOf(element)
					|| new TGroupChecker().isTypeOf(element)) {
				addDefinitions(zos, element);
